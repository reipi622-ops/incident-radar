    ...
    for report in unlinked:
        try:
            ...
            if result.matched_event_id:
                ...
                db.add(event)
                db.flush()  # get event.id before creating EventReport
            ...
            logger.error(f"Dedup error on report {report.id}: {e}", exc_info=True)
            db.rollback()  # reset broken session before next iteration
            errors += 1
