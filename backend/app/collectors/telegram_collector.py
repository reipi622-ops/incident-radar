...
class TelegramChannelCollector(BaseCollector):
    def __init__(self,channel,api_id,api_hash,limit=50,session="/tmp/tg"):
        self._ch=channel.lstrip("@");self._id=api_id;self._hash=api_hash;self._limit=limit;self._session=session
    @property
    def source_name(self):return f"telegram:{self._ch}"
    def collect(self):
        try:return asyncio.run(self._run())
        except Exception as e:logger.error(f"TG:{self._ch} {e}");return[]
    async def _run(self):
        ...
        async with TelegramClient(self._session,self._id,self._hash) as c:
            ...
            async for m in c.iter_messages(e,limit=self._limit):
                ...
                ts=m.date
                if ts and ts.tzinfo:ts=ts.astimezone(timezone.utc).replace(tzinfo=None)
                items.append(RawItem(external_id=f"{self._ch}_{m.id}",source_name=self.source_name,source_url=f"https://t.me/{self._ch}/{m.id}",raw_text=m.text,raw_timestamp=ts,media_items=[],language=_lang(m.text)))
        return items

def build_telegram_collectors():
    aid=os.getenv("TELEGRAM_API_ID","");ah=os.getenv("TELEGRAM_API_HASH","");chs=os.getenv("TELEGRAM_CHANNELS","")
    if not aid or not ah or not chs:return[]
